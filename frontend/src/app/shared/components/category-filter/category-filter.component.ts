  ...
  ngOnInit() {
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      if (this.type) {
        ...
        this.categoryWithTypes && this.categoryWithTypes.types && this.categoryWithTypes.types.length > 0
        && this.categoryWithTypes.types.some(type => this.activeParams.types.find
        (item => type.url === item)) ? this.open = true : this.open = false;
      }
    });
  }
